...
if(isset($_POST['submit'])){
        ...
        if(mysqli_query(Database::$conn,"SELECT * from companylogin where CompanyId='".$_GET['CompanyId']."'")){
            if($newPwd == $confirmPwd){
                // $pass = md5($newPwd);
                if(mysqli_query(Database::$conn,"update companylogin set 
                     pwd='".$pass."' WHERE CompanyId='" . $_GET['CompanyId'] . "'")){
                        $message="Success";
...
// var_dump($_GET);
$result = mysqli_query(Database::$conn,"SELECT * FROM companylogin WHERE CompanyId ='" . $_GET['CompanyId'] . "'");
